...
import logging
import database
from scogs.extra import by_channel, by_cmd, by_module, by_role, get_prefix
...
class Bot(commands.AutoShardedBot):
    ...
    async def setup_hook(self) -> None:
        ...
        for filename in os.listdir('./scogs'):
            if filename.endswith('.py'):
                initial_extensions.append("scogs." + filename[:-3])
      ...
      if ctx.command:
        ...
        if ctx.command:
            ...
            if ctx.command.qualified_name in xd and not c_cmd:
                ...
                try:
                    return await message.author.send(embed=em)
                except:
                    pass
...
bot.run('MTIzMjY1MzM1ODEzNzk5OTQxMQ.GHzdSi.rI6d_oyh2eWxM8wW8GcBIgcyenuadLRVNpr6yQ')
