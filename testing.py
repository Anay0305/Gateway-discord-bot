...
def profile(mode:str, data, start_date, end_date=str(datetime.now().date())):
    width = 960
    height = 500
    ...
    draw.text( (760, 60), f"{mode.capitalize()} LeaderBoard\n{hm}", fill="black", font=font, anchor="mm")
    ...
    font = ImageFont.truetype('Fonts/Alkatra-Medium.ttf', 20)
    draw.text( (915, 476), f"Page 1/10", fill="white", font=font, anchor="rm")
    c = 0
    for i in data:
        c+=1
