    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for i in self.values:
            ...
            if r in interaction.user.roles:
                await interaction.user.remove_roles(r, reason="Selfroles")
                removed.append(r.mention)
            else:
                await interaction.user.add_roles(r, reason="Selfroles")
...
class chooselabeloremoji(BasicView):
    ...
    @discord.ui.button(style=discord.ButtonStyle.primary, label="Label & Emoji", custom_id="L&E")
    async def dare(self, interaction, button):
        await interaction.response.defer(ephemeral=False, thinking=False)
        self.value = 'L&E'
        self.stop()
    ...
    async def truth(self, interaction, button):
        ...
        self.value = 'L'
        self.stop()
    @discord.ui.button(style=discord.ButtonStyle.green, label="Only Emoji", custom_id="E")
...
class selfroles(commands.Cog):
    ...
    async def selfrole(self, ctx: commands.Context):
        ...
        for i in sorted(ls):
            ...
            des += f"`{prefix}{i}`\n{cmd.description}\n\n"
        listem = discord.Embed(title=f"{emojis.cogs['Selfroles']} Selfroles Commands", colour=botinfo.root_color,
    ...
    async def show(self, ctx: commands.Context):
        no_panel = discord.Embed(description=f"There are no selfrole panels in this server", color=botinfo.root_color).set_footer(text=self.bot.user.name, icon_url=self.bot.user.avatar.url)
        self_db = database.fetchone("*", "srmain", "guild_id", ctx.guild.id)
        if self_db is None:
            ...
            if len(lsm) == 0:
                ...
                em = discord.Embed(title=f"Selfrole panels for {ctx.guild.name}", color=botinfo.root_color).set_footer(text=self.bot.user.name, icon_url=self.bot.user.avatar.url)
                d = ""
                for i in lsm:
                    if "placeholder" in i:
                        x = "DropDown"
    ...
    async def create(self, ctx: commands.Context):
        ...
        if self_db is None:
            ...
            lsb = literal_eval(self_db['data_button'])
            lsd = literal_eval(self_db['data_dropdown'])
        em = discord.Embed(description=f"Where should i send the panel of selfrole?\nIf you can't see any channel in the dropdown type its name in the dropdown selection box.", color=botinfo.root_color).set_footer(text=f"{self.bot.user.name} Selfroles", icon_url=self.bot.user.avatar.url)
        ...
        if val is None:
            ...
            while mcheck:
                ...
                if ty == "L&E":
                    ...
                    if samecb and samecl and len(x) != 0:
                        em.description = f"React with the emoji on this message for the button\nReact with {emojis.correct} if you dont want more buttons"
                        t = await init.add_reaction("{emojis.correct}")
                    else:
                        em.description = f"React with the emoji on this message for the button"
                    await init.edit(embed=em, view=None)
                    def check(reaction, user):
            ...
            if v7.value is None:
                    ...
                    for i in x:
                        ...
                        if r is not None:
                            if i['emo'] is not None and i['label'] is not None:
                                ...
                                des += f"> {i['label']}: {r.mention}\n"
                    des = "Interact with the following for getting or removing the role that follows\n\n" + des
                    emb = discord.Embed(description=des, color=botinfo.root_color)
                ...
                if emb:
                    main_embed = discord.Embed(description=v7.value, color=botinfo.root_color)
                    msg = None
                else:
                    main_embed = None
                    msg = v7.value
        ...
        if self_db is None:
            ...
            database.insert("srmain", "guild_id, 'data_button', 'data_dropdown'", val)
        else:
            dic = {
                'data_button': f"{lsb}",
                "data_dropdown": f"{lsd}"
            }
            database.update_bulk("srmain", dic, "guild_id", ctx.guild.id)
    ...
    async def edit(self, ctx:commands.Context, *, message_id):
        ...
        if self_db is None:
            ...
            if c == False:
                ...
                if msg is not None:
                    em = discord.Embed(title="Selfrole edit panel", color=botinfo.root_color)
                    em.description = f"Type your custom message"
