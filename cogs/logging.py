...
class enablemenu(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        cursor.close()
        db.close()
        em = discord.Embed(description=f"{channel.mention} is now set for {des[:-2]}", color=botinfo.root_color)
...
class disablemenu(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        db.close()
        if channel is not None:
            em = discord.Embed(description=f"Removed {des[:-2]} from being logged in {channel.mention}", color=botinfo.root_color)
        else:
            em = discord.Embed(description=f"Removed {des[:-2]} from being logged in the server", color=botinfo.root_color)
        await self.ctx.reply(embed=em)
        await interaction.message.delete()
        self.stop()
    ...
    async def _disable(self, interaction, button):
        ...
        if channel is not None:
            ...
            for i in sorted(ls1):
                        ...
                        pass
            db.commit()
            em = discord.Embed(description=f"Removed all loggings from {channel.mention}", color=botinfo.root_color)
        else:
            for i in sorted(ls):
                        sql = (f"UPDATE logs SET '{i}' = ? WHERE guild_id = ?")
                        val = (None, ctx.guild.id)
                        ...
                        pass
            db.commit()
            em = discord.Embed(description=f"Removed all loggings from the server", color=botinfo.root_color)
        await self.ctx.reply(embed=em)
        await interaction.message.delete()
        self.stop()
...
class logging(commands.Cog):
    ...
    async def logging(self, ctx):
        ...
        listem = discord.Embed(title=f"{emojis.cogs['Logging']} Logging Commands", colour=botinfo.root_color,
    ...
    async def config(self, ctx):
        ...
        db.close()
        ls = ["mod", "role", "message", "member", "channel", "server"]
        em = discord.Embed(title="Logs setting for the server", color=botinfo.root_color)
        for i in sorted(ls):
            if log_db[i] is not None:
    ...
    async def enable(self, ctx, *, channel: discord.TextChannel):
        ...
        em = discord.Embed(description=f"Which types of loggings should be logged in {channel.mention}?", color=botinfo.root_color)
    ...
    async def disable(self, ctx, *, channel: discord.TextChannel=None):
        ...
        if channel is not None:
            em = discord.Embed(description=f"Which types of loggings should be removed from being logged in {channel.mention}?", color=botinfo.root_color)
        else:
            em = discord.Embed(description=f"Which types of loggings should be removed from being logged?", color=botinfo.root_color)
        m = await ctx.reply(embed=em, view=view)
        await view.wait()
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        if not member.guild.me.guild_permissions.view_audit_log:
            return
        log_db = database.fetchone("*", "logs", "guild_id", member.guild.id)
        if log_db is None:
            return
        ...
        if member.bot:
            async for entry in member.guild.audit_logs(limit=1,after=datetime.datetime.now() - datetime.timedelta(minutes=2),action=discord.AuditLogAction.bot_add):
                ...
                m = entry.user
                em.add_field(name="Bot added by:", value=f"{m.mention}")
    ...
    async def on_member_remove(self, member: discord.Member):
        ...
        if kick:
            if log_db['mod'] is not None:
                ...
                em.add_field(name="Kicked by:", value=f"{m.mention}")
                if r:
    ...
    async def on_member_update(self, before: discord.Member,
        ...
        if not after.guild.me.guild_permissions.view_audit_log:
            return
        log_db = database.fetchone("*", "logs", "guild_id", after.guild.id)
        if log_db is None:
            return
        if log_db['member'] is not None:
            ...
            return
        em = discord.Embed(title="A member updated", color=botinfo.root_color)
        em.timestamp = datetime.datetime.utcnow()
        if after.nick != before.nick:
            async for entry in after.guild.audit_logs(
                    ...
                    action=discord.AuditLogAction.member_update):
                        ...
                        if not member.guild:
                            return
                        em.set_author(name=f"{str(member)}", icon_url=member.display_avatar.url, url=f"https://discord.com/users/{member.id}")
                        ...
                        em.add_field(name="Nick changed by:", value=f"{entry.user.mention}")
        ...
        if len(after.roles) != len(before.roles):
            async for ent in after.guild.audit_logs(
                ...
                if len(after.roles) > len(before.roles):
                    for r in after.roles:
                        ...
                        if r not in before.roles:
                            ...
                            em.add_field(name="Role Added by:", value=f"{ent.user.mention}")
                            x = "No"
                    ...
                    for r in before.roles:
                        ...
                        if r not in after.roles:
                            ...
                            em.add_field(name="Role Removed by:", value=f"{ent.user.mention}")
                            x = "No"
                            if after.top_role != before.top_role:
                                x = "Yes"
                            em.add_field(name="Top role changed?", value=x)
                ...
                em.set_author(name=f"{str(ent.target)}", icon_url=ent.target.display_avatar.url, url=f"https://discord.com/users/{ent.target.id}")
    ...
    async def on_member_ban(self, guild: discord.Guild, member: discord.Member):
        ...
        if not guild.me.guild_permissions.view_audit_log:
            return
        log_db = database.fetchone("*", "logs", "guild_id", guild.id)
        if log_db is None:
            return
        ...
        async for entry in guild.audit_logs(limit=1,
            ...
            em.add_field(name="Banned by:", value=f"{m.mention}")
            if entry.reason:
    ...
    async def on_member_unban(self, guild: discord.Guild, member: discord.Member):
        ...
        async for entry in guild.audit_logs(limit=1,
            ...
            em.add_field(name="Unbanned by:", value=f"{m.mention}")
            if entry.reason:
                em.add_field(name="Reason:", value=entry.reason)
            em.set_author(name=f"{str(member)}", icon_url=member.display_avatar.url)
    ...
    async def on_guild_role_create(self, role: discord.Role) -> None:
        ...
        if log_db['role'] is None:
            ...
            c = discord.utils.get(role.guild.channels, id=log_db['role'])
            if c is None:
                return
        async for entry in guild.audit_logs(limit=1,
                                            after=datetime.datetime.now() -
            ...
            if not m.guild:
                return
            em = discord.Embed(title="Role Created", description=f"Role {role.mention} Created by {entry.user.mention}", color=botinfo.root_color)
    ...
    async def on_guild_role_delete(self, role: discord.Role) -> None:
        ...
        async for entry in guild.audit_logs(limit=1,
            ...
            if not m.guild:
                return
            em = discord.Embed(title="Role Deleted", description=f"Role `{role.name}` Deleted by {entry.user.mention}", color=botinfo.root_color)
            em.add_field(name="Name", value=f"{role.name}")
            em.add_field(name="Colour", value=f"{role.color}")
            em.add_field(name="Mentionable", value=role.mentionable)
            em.add_field(name="Hoist", value=role.hoist)
    ...
    async def on_guild_role_update(self, before: discord.Role,
        ...
        async for entry in guild.audit_logs(limit=1,
            ...
            if not m.guild:
                return
            em = discord.Embed(title="Role Updated", description=f"Role {after.mention} Updated by {entry.user.mention}", color=botinfo.root_color)
    ...
    async def on_guild_channel_create(self, channel: discord.abc.GuildChannel) -> None:
        ...
        async for entry in guild.audit_logs(limit=1,
            ...
            if isinstance(channel, discord.TextChannel):
                em = discord.Embed(title="Text Channel Created", description=f"Text Channel {channel.mention} Created by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(channel, discord.VoiceChannel):
                em = discord.Embed(title="Voice Channel Created", description=f"Voice Channel {channel.mention} Created by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(channel, discord.CategoryChannel):
                em = discord.Embed(title="Category Created", description=f"Category Channel {channel.mention} Created by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(channel, discord.StageChannel):
                em = discord.Embed(title="Stage Channel Created", description=f"Stage Channel {channel.mention} Created by {entry.user.mention}", color=botinfo.root_color)
    ...
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel) -> None:
        ...
        async for entry in guild.audit_logs(limit=1,
            ...
            if not m.guild:
                return
            if isinstance(channel, discord.TextChannel):
                em = discord.Embed(title="Text Channel Deleted", description=f"Text Channel {channel.mention} Deleted by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(channel, discord.VoiceChannel):
                em = discord.Embed(title="Voice Channel Deleted", description=f"Voice Channel {channel.mention} Deleted by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(channel, discord.CategoryChannel):
                em = discord.Embed(title="Category Deleted", description=f"Category Channel {channel.mention} Deleted by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(channel, discord.StageChannel):
                em = discord.Embed(title="Stage Channel Deleted", description=f"Stage Channel {channel.mention} Deleted by {entry.user.mention}", color=botinfo.root_color)
            em.add_field(name="Name", value=f"{channel.name}")
            em.add_field(name="Position", value=channel.position + 1)
            em.add_field(name="Permissions synced?", value=f'{"Yes" if channel.permissions_synced else "No"}')
            if isinstance(channel, discord.VoiceChannel) or isinstance(channel, discord.StageChannel):
    ...
    async def on_guild_channel_update(self, before: discord.abc.GuildChannel,
        ...
        async for entry in guild.audit_logs(limit=1,
            ...
            if isinstance(after, discord.TextChannel):
                em = discord.Embed(title="Text Channel Updated", description=f"Text Channel {after.mention} Updated by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(after, discord.VoiceChannel):
                em = discord.Embed(title="Voice Channel Updated", description=f"Voice Channel {after.mention} Updated by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(after, discord.CategoryChannel):
                em = discord.Embed(title="Category Updated", description=f"Category Channel {after.mention} Updated by {entry.user.mention}", color=botinfo.root_color)
            if isinstance(after, discord.StageChannel):
                em = discord.Embed(title="Stage Channel Updated", description=f"Stage Channel {after.mention} Updated by {entry.user.mention}", color=botinfo.root_color)
            if before.name != after.name:
                em.add_field(name="Name changed", value=f"`{before.name}` to `{after.name}`")
            if isinstance(after, discord.TextChannel) and isinstance(before, discord.TextChannel):
                if before.topic != after.topic:
                    if before.topic is not None and after.topic is not None:
                        em.add_field(name="Channel's topic updated", value=f"`{'None' if before.topic is None or before.topic == '' else before.topic}` to `{'None' if after.topic is None or after.topic == '' else after.topic}`")
    ...
    async def on_guild_update(self, before: discord.Guild,
        ...
        async for entry in after.audit_logs(
            ...
            if not m.guild:
                return
            em = discord.Embed(title="Server Updated", description=f"Server Updated by {entry.user.mention}", color=botinfo.root_color)
            if before.name != after.name:
    ...
    async def on_message_edit(self, before: discord.Message, after:discord.Message):
        ...
        if log_db['message'] is None:
            ...
            if c is None:
                return
        em = discord.Embed(description=f":scroll: Message sent by {after.author.mention} edited in {after.channel.mention} [Jump to message]({after.jump_url})", color=botinfo.root_color)
    ...
    async def on_message_delete(self, message: discord.Message):
        ...
        async for entry in message.guild.audit_logs(limit=1, after=datetime.datetime.now() - datetime.timedelta(minutes = 1), action=discord.AuditLogAction.message_delete):
            em = discord.Embed(description=f":put_litter_in_its_place: Message sent by {message.author.mention} deleted in {message.channel.mention}", color=botinfo.root_color)
            url = None
    ...
    async def on_bulk_message_delete(self, messages: discord.Message):
        ...
        async for entry in guild.audit_logs(limit=1, after=datetime.datetime.now() - datetime.timedelta(minutes = 1), action=discord.AuditLogAction.message_bulk_delete):
            em = discord.Embed(description=f":put_litter_in_its_place: {len(messages)} Messages were deleted in {messages[0].channel.mention}", color=botinfo.root_color)
            x = datetime.datetime.now() - datetime.timedelta(seconds=5)
            em.add_field(name="**Deleted By:**",
                                value=f"{entry.user.mention} (ID: {entry.user.id})")
