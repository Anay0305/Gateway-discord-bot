...
class help(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction, command: Optional[str]):
        ...
        if command is not None:
            ...
            if cog is not None:
                if cog.qualified_name.capitalize() in emojis.snaps_cogs:
                    ...
                    em_list = []
                    no = 1
                    lss = emojis.snaps_cogs[command.capitalize()]
                    for k in ls1:
            ...
            pfp = xd.display_avatar.url
            v = ""
            for i in sorted(emojis.snaps_cogs):
                ls = emojis.snaps_cogs[i]
                v+=f"{ls} {i.capitalize()}\n"
            ...
            for i in sorted(emojis.snaps_cogs):
                ...
                for k in sorted(ls):
                    ok+= f"`{k}`, "
                lss = emojis.snaps_cogs[i.capitalize()]
            ...
            no = {}
            count = 1
            for i in emojis.snaps_cogs:
                no[i] = count
                count+=1
            database.insert("help", "main, 'no'", (x, 0))
            page = HPaginationView(embed_list=em_list, no=no, cogs=emojis.snaps_cogs, i=x, ctx=ctx)
    ...
    async def command_autocomplete(self, interaction: discord.Interaction, needle: str) -> List[app_commands.Choice[str]]:
        ...
        if not needle:
            return [
                ...
                for cog_name in sorted(emojis.snaps_cogs)
            ][:25]
        needle = needle.lower()
        return [
