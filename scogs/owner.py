...
class owner(commands.Cog):
    ...
    async def snapsgw(self, ctx: commands.Context, message_id, *, member: discord.User = None):
        await ctx.message.delete()
        if ctx.guild.id != 1213915680345432174:
            return
        gw_db = database.fetchone("*", "gwmain", "guild_id", 1183413203376554054)
        if gw_db is None:
            return await ctx.send(f"Invalid Giveaway id")
        xdd = literal_eval(gw_db["gw"])
        if int(message_id) not in xdd:
            return await ctx.send(f"Invalid Giveaway id")
        xd = xdd[int(message_id)]
        if 'winner_ids' in xd:
            if member is not None:
                if len(xd['winner_ids']) >= xd['winners']:
                    return await ctx.send(f"All the winners of this giveaway are already decided")
                if member.id in xd['winner_ids']:
                    xd['winner_ids'].remove(member.id)
                else:
                    xd['winner_ids'].append(member.id)
            else:
                em = discord.Embed(color=botinfo.root_color, title=f"{xd['prize']}'s Winners")
                em.set_footer(text="Giveaway")
                count = 0
                des = []
                for i in xd['winner_ids']:
                    count+=1
                    u = discord.utils.get(self.bot.users, id=i)
                    des.append(f"{count} - {u.mention}")
                em.description = '\n'.join(des)
            ...
            if member is None:
                em = discord.Embed(color=botinfo.root_color, title=f"{xd['prize']}'s Winners")
                em.set_footer(text="Giveaway")
                em.description = "No pre decided winner for this giveaway"
                return await ctx.send(embed=em)
            xd['winner_ids'] = [member.id]
        xdd[int(message_id)] = xd
        database.update("gwmain", "gw", f"{xdd}", "guild_id", 1183413203376554054)
